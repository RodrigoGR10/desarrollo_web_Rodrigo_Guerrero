...
const redesData = ["Whatsapp", "Telegram", "X", "Instagram", "TikTok", "Otra"];

const poblarRedes = () => {
  const selects = document.querySelectorAll(".select-course");
  selects.forEach(sel => {
    sel.innerHTML = '<option value="">Seleccione una red</option>';
...
function changeArguments(e) {
  ...
  const label = document.querySelector("label[for='Id_URL_" + suffix + "']");
  const input = document.getElementById("Id_URL_" + suffix);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let i = 1; i <= 5; i++) {
    const sel = document.getElementById("select-course-" + i);
    if (sel) sel.addEventListener("change", changeArguments);
  }

  for (let i = 1; i <= 5; i++) {
    const sel = document.getElementById("select-course-" + i);
    if (sel) changeArguments({ target: sel });
  }
});